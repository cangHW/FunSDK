...
class MultiTaskWorker(task: DownloadTask) : BaseWorker(task) {
    ...
    init {
        ...
        latch = CountDownLatch(getTaskNum())
    }
        ...
        CsTask.ioThread()?.call(object : ICallable<String> {
            ...
            override fun apply(value: String): String {
                ...
                getAllTaskMsg().forEach {
                    if (it is MultiMsg) {
                        CsLogger.tag(tag)
                            .i("开始合并分片. taskTag = ${task.getTaskTag()}, part: ${it.partPosition}, totalPart: ${getTaskNum()}")
                        if (
                            !CsFileWriteUtils.setSourcePath(it.filePath).writeSync(tempPath, true)
                        ) {
                            CsFileUtils.delete(tempPath)
                            CsLogger.tag(tag)
                                .i("合并分片失败. taskTag = ${task.getTaskTag()}, part: ${it.partPosition}, totalPart: ${getTaskNum()}")
                            callbackEnd(false, IllegalArgumentException("合并文件失败"))
                            return TASK_FINISH
                        }
                        CsLogger.tag(tag)
                            .i("分片合并成功. taskTag = ${task.getTaskTag()}, part: ${it.partPosition}, totalPart: ${getTaskNum()}")
                    }
                }
                return value
            }
        ...
        disposable = CsTask.computationThread()?.delay(1, TimeUnit.SECONDS)
            ?.doOnNext(object : IConsumer<Long> {
                override fun accept(value: Long) {
                    val list = getAllTaskMsg().filter { it.stream != null }
    ...
    private fun startTask(msg: MultiMsg) {
        CsTask.ioThread()?.call(object : ICallable<String> {
            override fun accept(): String {
                //1.任务开始
                CsLogger.tag(tag)
                    .i("准备下载分片 taskTag: ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
                if (CsFileUtils.length(msg.filePath) == msg.partSize) {
                    CsLogger.tag(tag)
                        .i("分片已存在且满足要求 taskTag: ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
            ...
            override fun apply(value: String): String {
                ...
                CsLogger.tag(tag)
                    .i("创建分片文件 taskTag = ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
                if (!CsFileUtils.createFile(msg.filePath)) {
                    CsLogger.tag(tag)
                        .i("创建分片文件失败 taskTag = ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
            ...
            override fun apply(value: String): String {
                ...
                CsLogger.tag(tag)
                    .i("开始下载分片 taskTag = ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
                try {
                    Task.start(
                        task,
                        msg.filePath,
                    ...
                    if (CsFileUtils.length(msg.filePath) == msg.partSize) {
                        CsLogger.tag(tag)
                            .i("分片下载成功 taskTag = ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
                        msg.status = STATUS_SUCCESS
                        return TASK_RUNNING
                    }
                } catch (throwable: Throwable) {
                ...
                CsLogger.tag(tag)
                    .e("分片下载失败 taskTag = ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
                //下载失败
                msg.status = STATUS_FAILED
                return TASK_FINISH
            }
            ...
            override fun onCallback() {
                //4.执行后续分片任务
                CsLogger.tag(tag)
                    .i("分片下载结束 taskTag = ${task.getTaskTag()}, part: ${msg.partPosition}, totalPart: ${getTaskNum()}")
                runningNum.decrementAndGet()
                tryStartTask()
                latch?.countDown()
            }
